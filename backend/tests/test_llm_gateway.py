"""LLM gateway — metering at the single boundary.
...
def _wire(monkeypatch, provider_id, capture):
    ...
    monkeypatch.setattr(
        ...
        lambda: {"llm_provider": provider_id, "llm_model": "gpt-4o"},
    )
    monkeypatch.setattr(queries, "record_llm_run", lambda row: capture.append(row))


def test_cloud_complete_sends_original_and_meters(monkeypatch):
    """Cloud providers receive the original message — no redaction applied."""
    rows: list[dict] = []
    _wire(monkeypatch, "openai", rows)  # openai = cloud
    content = "Email a@b.com, call +1 415 555 0132. Skill: Python."
    reply = llm.complete([{"role": "user", "content": content}])
    assert reply.startswith("Dear team")
    sent = _Fake.last_sent[0]["content"]
    # Original content preserved — no placeholders injected
    assert sent == content
    assert "[redacted-email]" not in sent
    assert "[redacted-phone]" not in sent
...
def test_local_complete_sends_original(monkeypatch):
    rows: list[dict] = []
...
def test_stream_passes_tokens_through_and_records(monkeypatch):
    ...
    assert _Fake.last_sent[0]["content"] == "hi a@b.com"
